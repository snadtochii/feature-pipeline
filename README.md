...
| `/feature:setup` | Configure a project in one guided run: detect its commands, ask what detection leaves open, and write `config.yaml`, the ticket folders, `.worktreeinclude` and optional `.gitignore` / `## Commands` lines, each after an approved diff. Re-runs keep what exists. See [Configuration](#configuration). |
