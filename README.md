...
| **Ship** *(standalone)* | `ship` | implementer + independent reviewer subagents | Manual | Autonomous build → independent review loop over a ticket or `blocked_by` chain, ending at an open PR. Each ticket: an implementer subagent runs `/feature:flow --pr --no-ui-testing`, an independent reviewer posts findings, the implementer addresses them. A chain/epic squash-merges per-ticket PRs into an `integration/<epic-id>` branch; the resulting PR (solo single-ticket PR, or integration→base) is left open for human review by default — `--merge` lands it. Invoked directly — not a pipeline stage |
