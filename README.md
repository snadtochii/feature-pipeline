...
| **Sync** *(standalone)* | `sync` | — (runs inline; reads PR state via `gh`) | Manual or `/loop` | Reconcile `review/` tickets with GitHub PR state: scan `review/`, promote merged-PR tickets to `done/` (Transition 6), report open ones, flag closed-unmerged. Read-only on GitHub; invoked directly — not a pipeline stage |
