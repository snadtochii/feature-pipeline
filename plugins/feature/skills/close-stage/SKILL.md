...
Every message in this stage re-reads its whole window (the runtime reference's tool-result rule: several tool calls in one message re-read it once), so the numbered steps below are the message sequence, and a skipped checkpoint closes in five messages after the skill load: the ticket resolution (step 3), the preparation read (step 4), one artifact write (§1 step c through §3), the finalizer spawn (§5) and the final message (§6). Nothing below reads a reference on its own — what the stage needs is fetched inside the preparation read, and what it writes is inlined in this file. Any `error` below ends the stage with the Result line.
