...
argument-hint: "[ticket-id] [--pr] [--no-commit] [--no-ui-testing] [--attach-screenshots]"
---
...
Its user-facing stops are the verdict gate's and a finalizer `needs-decision` relayed through it. Standalone, the stage asks them in the main conversation; under `flow`, or under standalone `build`'s stage chain, it runs as a stage subagent, and each stop pauses it while that sequencer relays the question. `ui-tester` and `finalizer` are its leaf children.
...
**Runtime.** Bind the runtime reference, the plugin root and the project or worktree root from the brief's runtime block when there is one — [`stage-briefs.md`](../flow/references/stage-briefs.md) §3 makes it authoritative, so no runtime file is read — else, standalone (`/feature:close-stage` in the main conversation), per [../flow/references/runtime.md](../flow/references/runtime.md). Use the bound runtime's operations for every role spawn — the UI tester and the finalizer: prefix their complete prompts with the runtime block and honor its capacity policy and role boundaries.

Every message in this stage re-reads its whole window (the runtime reference's tool-result rule: several tool calls in one message re-read it once), so the numbered steps below are the message sequence, and a skipped checkpoint closes in five messages after the skill load when no lesson is captured, six when one is: the ticket resolution (step 3), the preparation read (step 4), the lesson's supersession check when a lesson is drafted (§3), one artifact write (§1 step c through §3), the finalizer spawn (§5) and the final message (§6). A browser pass costs more, because the `## Screenshots` section's inputs arrive before the artifact body is composable: the tester spawn, then the evidence listing, then the `05-tests.md` write — one message more than the tester's return in fs-native, two in server-native, where the asset upload and the parse of its output sit between them — one when the storage file rules that upload out before it runs (§1 step b). Nothing below reads a reference on its own — what the stage needs is fetched inside the preparation read, and what it writes is inlined in this file. The one exception is [`../build/references/ui-attach.md`](../build/references/ui-attach.md), read on every browser pass because that section applies its §3 order and §4 alt grammar, and it rides the evidence-listing message (§1 step b). Any `error` below ends the stage with the Result line.
...
4. **The preparation read** — one message of parallel tool calls, its contents the list in §1 of the storage file now in hand; no artifact goes through a Bash print (the runtime reference's tool-result rule). It binds the working copy: `<ticket-folder>` is absolute and stays in the main checkout. Nothing it fetches is read a second time later in the stage. The spec's acceptance-criteria section (the per-criterion lines of `05-tests.md`) and the base branch (§5) ride it, per §1 of the storage file, so neither goes out in a call of its own. The lessons store is not read here: its only read is the write-time supersession check (§3). The rest of `01-spec.md`'s body is read only when a tester spawn needs it and the preparation read did not already carry it, and `02-plan.md` only for the skip-detection scan.

The remaining steps are computed from that read and call nothing:

5. **Epic refusal.** Step 4 of the ticket-resolution reference: `kind: epic` → `error`, `failed-step: ticket`, instructing the user to close a child ticket instead — epics are non-pipelineable. Blockers are not re-checked: build validated them before any code was written.
...
7. **Bind the commit mode — same placement rule.** Bind `commit_mode` from the `git.commit` key of the optional `git:` block in `claudedocs/tickets/config.yaml`, from the preparation read's copy of the file — never `yq`/`jq` (the file is a local repo file whatever the storage mode). Values: `prompt`, `always`, or `never`. Missing file, missing block, missing key, or `prompt` → `prompt`. Any other value → `prompt`, plus a one-line notice printed now — `git.commit: <value> is not a recognized mode (prompt | always | never) — treating as prompt.` — a config error never blocks a close. Then apply the per-run flags (the `--pr`+`--no-commit` pair was already rejected at step 2):
   ...
   Bind `attach` in the same step, from the same copy of the file: on when `git.attach_screenshots` is `true` or `--attach-screenshots` was passed, else off — missing file, block or key → off; any other value → off, plus the notice `git.attach_screenshots: <value> is not true|false — treating as false.` The flag only turns it on ([`../build/references/ui-attach.md`](../build/references/ui-attach.md) §1). `--attach-screenshots` without `--pr` prints `--attach-screenshots has no effect — no PR in this run.` and binds nothing. `attach` is consumed only by the summary write's posted-body files (§3) and the finalizer prompt (§5). The test checkpoint's evidence listing and its `ui-attach.md` read are not among its consumers: both happen on every browser pass, because the `## Screenshots` section needs them (§1 step b).
...
| `06-summary.md` exists with verdict `partial` or `stuck`, the ticket's own `status` is neither `in-progress` nor `in-review`, and `06-summary.md` is not older than any of `02-plan.md`, `03-implementation.md`, `04-review.md` and `05-tests.md` | Already closed — the gate's decision was applied (`accept-as-partial`, `abort`, or `continue-with-hint` before its implement round started). Print "`<ticket-id>` already closed (`<status>`, verdict `<v>`). Re-run implement with a hint to continue, or delete `03-implementation.md` onward to rebuild." and return `already-complete — verdict <v>`. No gate is re-presented and nothing is written. |
| `06-summary.md` exists, the ticket's own `status` is `in-progress`, and `06-summary.md` is not older than any of `02-plan.md`, `03-implementation.md`, `04-review.md` and `05-tests.md` | Incomplete tail — the finalizer never ran, or returned an `error`. Only `in-progress` marks a pending gate: implement's Transition 1 resets a hint round's `partial-completion` to `in-progress` before any code, so a `partial-completion` ticket whose summary is current has had its decision applied. Run steps 8–9, then re-enter at the verdict gate (§4) with the verdict on `06-summary.md`'s first line: re-present the gate and re-collect the decision. A `pass` ending that asks nothing — `--pr`, `git.commit: always`, `never` or `--no-commit` — goes straight to the finalizer spawn (§5). The finalizer is idempotent, so an already-made commit, an already-pushed branch, an already-open PR or an already-fired transition is detected and reported rather than repeated. |
...
| `05-tests.md` newer than `04-review.md`, with a `## Failed Criteria` section | Run steps 8–9, then re-enter the fix loop (§1 step d) with the recorded failed criteria and a fresh budget. The pre-flight state is not persisted, so the loop re-runs the pre-flight before its first tester re-spawn. Under `--no-ui-testing` the loop cannot re-verify a fix, so it is not entered: the recorded failures stay under `## Failed Criteria`, and the stage goes straight to §2, where they yield `partial`. |
| Otherwise | Run steps 8–9, then a fresh close from §1. An older `05-tests.md` or `06-summary.md` is overwritten. |
...
**Reachability pre-flight (per [`references/test-preflight.md`](references/test-preflight.md)).** When step a matched UI signals (and `--no-ui-testing` was not set), run the pre-flight gate *before* spawning the `ui-tester` — the cheap `curl` is always paid first. It resolves a URL (`test.url` → project `CLAUDE.md` → common-port probe), `curl`s it (reachable iff HTTP `200/301/302/401/403`), and on an unreachable app optionally boots a declared `test.start` (backgrounded, poll bounded by `test.start_timeout`, default 60s) that it then owns for teardown:
   ...
   The pre-flight reads the `test:` block by model-reading `claudedocs/tickets/config.yaml`; it never invokes `yq`/`jq`. Absent a `test:` block, URL resolution falls through to the CLAUDE.md → port-probe path and no `test.start` is booted.
...
b. **Spawn `feature:ui-tester`** (when reachable). Read the project's `CLAUDE.md` for a test framework hint (`## Testing` section, `## Commands` section, or inline references like "Playwright specs in `e2e/`"). One role child through the selected runtime:
   ...
   Save subagent output to `<ticket-folder>/05-tests.md` **as soon as the composition inputs are in hand** — one message after the tester returns in fs-native, one or two in server-native — and before any fix, so a re-run finds the failed criteria on disk. Failed criteria become a `## Failed Criteria` section inside `05-tests.md`; the injected block's §5 decides how required-check findings and failed checks are listed there, so a layout failure affects the verdict like any other failed criterion. If specs were codified, list their paths under a `## Codified specs` section.

   **The `## Screenshots` section.** Every browser pass — `attach` on or off, `--pr` or not — lists the evidence home's `*.png` files with their sizes in one Bash call, `wc -c -- "<evidence-home>"/*.png` with the directory quoted, and the same message reads [`../build/references/ui-attach.md`](../build/references/ui-attach.md), whose §3 order and §4 alt grammar the section applies. That read is no longer conditional: the section consumes those two sections on every pass. Compose the section from the listing per [`references/storage-fs.md`](references/storage-fs.md) / [`references/storage-server.md`](references/storage-server.md) §8, for the mode detected at stage start, and write it inside the `05-tests.md` body. The listed names and sizes are inputs to that body, so the listing cannot ride the write: in fs-native the sequence is listing → compose and write, one message more than the tester's return; in server-native it is listing plus the CLI probe → upload and parse → compose and write, two messages more — one when §8 rules the upload out from the listing's own results, in which case the composition follows the listing directly. The attach path keeps its own gating unchanged — the §3 selection that feeds the summary write (§3) and the manifest reason `evidence path not attach-safe` apply only when `attach` is on **and** `--pr` was passed, and §4's path gate guards an argv, never the section.

   A tester crash or timeout is recorded in `05-tests.md` under `## Failed Criteria` — the crash or timeout named, and every acceptance criterion it left unverified listed — and the `## Screenshots` section is still written, from whatever the evidence home holds, with the `No captures were written.` line when it holds nothing; the stage then proceeds to the verdict without entering the fix loop, so the verdict is `partial`. Artifact verdict for this write: §4.
...
d. **Fix loop — at most 2 iterations.** Failed criteria are observations the stage consumes inline, within a budget. The fix-loop budget is 2 iterations per invocation, never persisted; a re-entry through the Router starts a fresh one. One iteration:
   1. Fix every failed criterion inline, smallest change first, building nothing beyond `02-plan.md` and the failed criteria. Every content edit to a project source file goes through the runtime's structured edit tool ([`../flow/references/runtime-claude.md`](../flow/references/runtime-claude.md) / [`../flow/references/runtime-codex.md`](../flow/references/runtime-codex.md) §File edits) — single-file edits without exception, whatever the session's permission mode allows.
   2. **Validate after each edit.** The validation commands are resolved at the first fix: read the project instruction files for **both** runtimes — `CLAUDE.md` and `AGENTS.md` — for lint and typecheck commands (a `## Commands`, `## Validation` or `## Testing` section, or inline references), the current runtime's primary file first (`AGENTS.md` when `$PLUGIN_ROOT` is set and `$CLAUDE_PLUGIN_ROOT` is not, else `CLAUDE.md`), then the other; a file the runtime already loaded as project instructions is taken from context, not re-read. Run them as `cd "<root>" && <command>`. None documented → log one line (`No validation commands found in project CLAUDE.md/AGENTS.md — proceeding without skill-body validation`) and continue. They are lint and typecheck only, a narrower set than build's per-step chain — [`../build/references/validation-chain.md`](../build/references/validation-chain.md) explains the split.
   3. **A fix that will not validate, or cannot be made cleanly, is reverted** with the same structured edit tool, restoring its pre-edit text; the criterion stays failed. Never use `git checkout`, `git restore` or `git stash` on a file: the implement phase's uncommitted work lives in the same files.
   ...
   5. Rewrite `05-tests.md` with the result, its `## Screenshots` section recomposed from a fresh listing of the evidence home — and in server-native from a re-upload of the current candidate set, since the server overwrites an asset by name (§8) — so the links point at the bytes this iteration produced (artifact verdict: §4).

   **Watch for stuck patterns** 1–5 in [`../build/references/stuck-detection.md`](../build/references/stuck-detection.md) (action↔observation repetition, action↔error repetition, monologue, ping-pong, repeated context errors) throughout the loop. On detection, stop fixing and mark the verdict `stuck`, naming the pattern. The arbiter (pattern 6) does not fire here; the 2-iteration budget — the close stage's hard maximum in the same reference — bounds the loop.
...
**The screenshot pointer line.** When a browser pass ran, the body carries one line — `Screenshots: <n> captures, listed in 05-tests.md` — where `<n>` is the total the `## Screenshots` section lists, never the uploaded count. It is plain text: no link syntax, no path, no URL. The reason is the paragraph below — this body becomes the PR body, the commit body and a sibling's blocker context, where a relative link would dangle and a hosted URL would be noise for readers who cannot open it. A skip variant adds no line.
...
**Posted-body files — only for `pass` with `--pr`, `attach` on, and a browser pass whose evidence home holds a recognized capture.** In the same call as the `06-summary.md` write, write the two PR-body variants of [`../build/references/ui-attach.md`](../build/references/ui-attach.md) §5, each whole — the attach variant `pr-body-attach.md` and the manifest variant `pr-body-manifest.md`, where §6 of the storage file puts them. Each is the `06-summary.md` content, preceded by the epic lead line of [`../build/references/pr-creation.md`](../build/references/pr-creation.md) §4 for an epic child, followed by the `## UI evidence` section, its marker keyed on the ticket ID. The §3 selection runs here over the size listing §1 step b already collected, and the §4 path gate's result from that step applies: a gate failure, or a selection with nothing attachable, writes the manifest variant alone and records the reason for the finalizer prompt. `06-summary.md` itself never carries the `## UI evidence` section — that section exists only in these two posted-body files; the plain-text pointer line above is a separate thing and does live in the summary body. A skip artifact or a crashed tester writes neither file, and a re-run rewrites both.
...
- The supersession check (contract §4) is the stage's one read of the store — the keyword grep, or the listing with its in-memory keyword match, per §5 of the storage file. It goes out once, after the lesson is drafted, in the call before the summary write: on a skip that call also carries step 9's chained existence check; after a browser pass it is the last message before the summary write. The lesson line itself is written with the summary. No lesson to capture → no read of the store.
...
- A close run is **unattended** in the §6/§7 sense — no human is reachable to answer, so the CLAUDE.md proposals are skipped — under headless `claude -p`, or as a subagent whose brief says no human is reachable (the close brief carries that line, set from how its sequencer — flow, or standalone build — was invoked). A stage subagent whose brief says a human is reachable is attended: it pauses with the proposal and continues with the relayed answer.
...
**The base branch** for the prompt is the line the preparation read printed (§1 of the storage file). Only when that line is empty is it resolved again with [`../build/references/pr-creation.md`](../build/references/pr-creation.md) §1's short-name helper (`git symbolic-ref --short refs/remotes/origin/HEAD | sed 's@^origin/@@'`, falling back to `main`), path-bound to `<root>` and chained onto step 9's call — never issued alone.

**Spawn [`feature:finalizer`](../../agents/finalizer.md)** — exactly one child, on each of the three spawning endings, through the selected runtime. Its spawn `description` is the literal `Finalize <TICKET-ID>`. The prompt is self-contained: every path in it is **absolute**, every mode-specific fact is the value the stage's own `-fs`/`-server` file resolved, and it carries no relative link — the child does not share the stage's context and cannot resolve a path against this skill's directory. The one mode file it names, the state-transitions reference for the bound mode, is named by absolute path and section so the child reads the transition mechanics itself instead of the stage restating them at its own, larger window. Substitute every `<…>` below with the resolved value before sending; `<PLUGIN_ROOT>` is the root bound at Entry.

   > Finalize the post-gate mechanics for `<TICKET-ID>` — they are already decided; perform them, never re-decide them. Storage mode: `<fs-native | server-native>`. Ticket folder (absolute): `<ticket-folder>`. `06-summary.md`: `<absolute path>`. `01-spec.md`: `<absolute path>`. `<In server-native, additionally: the ticket handle, and the absolute scratchpad paths of the pulled 06-summary.md, the id file and the title file — pull nothing yourself.>` Verdict: `<pass | partial | stuck>`. Ending: `<pass | pass --pr | accept-as-partial | abort>`. Commit: `<commit per the commit.md conventions | leave everything uncommitted>`. `<--pr overrides the project's git.commit: never — commit and push anyway.>` Base branch: `<base>`. Session-state path to exclude from staging: `<the project config's resolved test.auth.storage_state, absolute | none declared>`. Epic slug for the PR body lead: `<the epic: slug | not an epic child>`. Screenshots: `<none | manifest only — posted body <absolute path of pr-body-manifest.md>, reason <the §3 reason> | attach — posted body (attach) <absolute path of pr-body-attach.md>, posted body (manifest) <absolute path of pr-body-manifest.md>, attach list, in order: "<absolute path>#<alt>", …>` — both posted bodies already carry the epic lead line, so none is prepended to them. Worktree: `<bound — wt-path <wt-path>, branch <branch>, repo root <repo-root>; teardown row: <the row named above> | none bound>`.
   >
   > **Your mechanics references, at these absolute paths** — read the named sections only, all in one message before you act: `<PLUGIN_ROOT>/skills/build/references/commit.md` (all of it); `<PLUGIN_ROOT>/skills/build/references/pr-creation.md` §0–§5 (skip its Merge predicate — that is the caller's, not yours); `<PLUGIN_ROOT>/skills/build/references/worktree.md` §2 step 4, §3 and §4; `<PLUGIN_ROOT>/skills/flow/references/state-transitions-<fs|server>.md` — the `## Transition <N>` section of each transition named below, plus `## Epic-completion predicate` when the ticket is an epic child; and, only when Screenshots is not `none`, `<PLUGIN_ROOT>/skills/build/references/ui-attach.md` §2, §6 and §7.
   >
   ...
   >
   > **Transition(s) to apply — decided; perform each per its `## Transition <N>` section in the reference**: `<in order: number and name — e.g. "Transition 5 — Open-PR", then "Transition 2 — End-of-pipeline" as the degradation swap>`. Where: `<fs-native: the ticket store root (the absolute claudedocs/tickets/ path in the main checkout) and the ticket's current folder (the ticket folder above; its frontmatter file is 01-spec.md there) | server-native: the ticket handle>`. `<When this ticket is an epic child: "Epic child of <EPIC-ID>" plus <fs-native: the epic's absolute folder and its prd.md path | server-native: the epic's handle> — after your own status flip, read the roster from that epic and evaluate the reference's Epic-completion predicate; report any warning it raises in `notes`, since the close stage has no other channel to surface it.>` The section supplies the mechanics for both modes — the Solo / Child rows, the folder-move-then-frontmatter ordering, the status values and the CAS `from[]`; this prompt names only which transitions and where.
   >
...
Composed from the finalizer's `ok` result — its `transition`, `commit`, `pr`, `branch`, `worktree` and `notes` fields — and printed once the child returns. Print every `notes` line beneath the transition line: staging exclusions, Epic-completion warnings, degradations, already-done steps and the `Screenshots:` outcome line with its upgrade hint ([`../build/references/ui-attach.md`](../build/references/ui-attach.md) §7) reach the user through no other channel, and a silent exclusion is indistinguishable from a guard that never fired.
...
- Whenever the result carries a `pr` URL, this line — the only copy of it; [`../build/references/pr-creation.md`](../build/references/pr-creation.md) §5 points here — filled from the result's `pr` and `branch` and the `<base>` the stage passed to the finalizer, nothing re-read to compose it: `✅ PR opened: <url>  (branch <branch> → <base>). Ticket → review/. Merge the PR, then re-run to finalize to done/.` Keyed on the field rather than on the `review/` transition, so a caller whose overrides suppressed the transition still gets the PR reported.
