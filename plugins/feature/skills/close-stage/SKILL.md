...
**Spawn [`feature:finalizer`](../../agents/finalizer.md)** — exactly one child, on each of the three spawning endings, through the selected runtime. Its spawn `description` is the literal `Finalize <TICKET-ID>`. The prompt is self-contained: every path in it is **absolute**, every mode-specific fact is the value the stage's own `-fs`/`-server` file resolved, and it carries no relative link — the child does not share the stage's context and cannot resolve a path against this skill's directory. The one mode file it names, the state-transitions reference for the bound mode, is named by absolute path and section so the child reads the transition mechanics itself instead of the stage restating them at its own, larger window. Substitute every `<…>` below with the resolved value before sending; `<PLUGIN_ROOT>` is the root bound at Entry.
   ...
   >
   > **Transition(s) to apply — decided; perform them per the reference's section for each**: `<For each transition in the decision, in order: its number and name — e.g. "Transition 5 — Open-PR", then "Transition 2 — End-of-pipeline" as the degradation swap.>` Ticket store root (absolute): `<the absolute claudedocs/tickets/ path in the main checkout | the server project>`; the ticket's current folder is the ticket folder above and its frontmatter file is `01-spec.md` there. `<In server-native, instead: the ticket handle, the target status, and the CAS `from[]` value — never widened.>` `<When this ticket is an epic child, additionally: "Epic child of <EPIC-ID> — epic folder (absolute): <path>, its prd.md: <path>. After your own status flip, read the roster from that prd.md's `children:` and evaluate the reference's Epic-completion predicate; report any warning it raises in `notes`, since the close stage has no other channel to surface it."> The reference's Solo / Child rows, its folder-move-then-frontmatter ordering and its predicate are the mechanics; this prompt names only which transitions and where.
   >
