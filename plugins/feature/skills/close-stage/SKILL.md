...
Every message in this stage re-reads its whole window (the runtime reference's tool-result rule: several tool calls in one message re-read it once), so the numbered steps below are the message sequence, and a skipped checkpoint closes in five messages after the skill load when no lesson is captured, six when one is: the ticket resolution (step 3), the preparation read (step 4), the lesson's supersession check when a lesson is drafted (§3), one artifact write (§1 step c through §3), the finalizer spawn (§5) and the final message (§6). Nothing below reads a reference on its own — what the stage needs is fetched inside the preparation read, and what it writes is inlined in this file. The one conditional read, [`../build/references/ui-attach.md`](../build/references/ui-attach.md) when screenshots will be attached, rides the message that saves `05-tests.md` (§1 step b). Any `error` below ends the stage with the Result line.
...
b. **Spawn `feature:ui-tester`** (when reachable). Read the project's `CLAUDE.md` for a test framework hint (`## Testing` section, `## Commands` section, or inline references like "Playwright specs in `e2e/`"). One role child through the selected runtime:
   ...
   Save subagent output to `<ticket-folder>/05-tests.md` **as soon as the tester returns**, before any fix, so a re-run finds the failed criteria on disk. Failed criteria become a `## Failed Criteria` section inside `05-tests.md`; the injected block's §5 decides how required-check findings and failed checks are listed there, so a layout failure affects the verdict like any other failed criterion. If specs were codified, list their paths under a `## Codified specs` section. When `attach` is on and `--pr` was passed, the same message — and each fix iteration's rewrite — also lists the evidence home's `*.png` files with their sizes in one Bash call — `wc -c -- "<evidence-home>"/*.png`, the path already past [`../build/references/ui-attach.md`](../build/references/ui-attach.md) §4's gate, else no listing and the manifest reason `evidence path not attach-safe` — and, on the first save only, reads [`../build/references/ui-attach.md`](../build/references/ui-attach.md), whose §3 selection and §4 alt text the summary write applies. A tester crash or timeout is recorded in `05-tests.md` under `## Failed Criteria` — the crash or timeout named, and every acceptance criterion it left unverified listed — and the stage proceeds to the verdict without entering the fix loop, so the verdict is `partial`. Artifact verdict for this write: §4.
