...
**The preparation read** (the skill's Entry step 4) is one message carrying every call in parallel: Read ticket metadata; List artifacts (the rows' `updated_at` — §7's signals); Read artifact for each of `01-spec.md`, `03-implementation.md`, `04-review.md`, `05-tests.md` and `06-summary.md` — a read of an artifact the ticket does not have fails, and that failure is its absence, not a §9 error; `pipeline_list_lessons` (the rows the lessons contract's §4 scans); and one Bash call printing this file, `claudedocs/tickets/config.yaml` and the lessons contract, [`lessons-log-server.md`](../../flow/references/lessons-log-server.md). `02-plan.md` is pulled only when the skip-detection scan needs it. The pulled bodies are the working copies: each lands in the scratchpad with the first §3 write that touches it, and a body the stage never modifies is never materialized.
