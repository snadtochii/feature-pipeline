...
- **Incomplete tail** — the finalizer never ran, or returned an error — keyed on `06-summary.md` being present in the listing while the row status is still `in-progress` or `partial-completion`, and its `updated_at` is not earlier than that of any of `03-implementation.md`, `04-review.md` and `05-tests.md`. Both status values are needed: `accept-as-partial` sets `partial-completion` in its own operation, before the one that lands the terminal status, so a tail interrupted between them carries the former. A summary older than any of those three belongs to an earlier close run that a later implement or review pass has since superseded, so a fresh close owns the ticket. The gate's decisions are conversational state and are deliberately not persisted, so nothing else about the tail is recoverable from the store.
