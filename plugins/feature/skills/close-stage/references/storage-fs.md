...
**The preparation read** (the skill's Entry step 4) is one message carrying these tool calls in parallel: one Bash call, run from the project root, that prints `claudedocs/tickets/config.yaml`, the frontmatter of `01-spec.md`, its acceptance criteria (`sed -n '/^## Acceptance Criteria/,/^## /p'` over `01-spec.md` — the per-criterion lines `05-tests.md` records; a spec without the section prints nothing), `ls -l` over `02-plan.md`, `04-review.md`, `05-tests.md` and `06-summary.md` (presence and mtimes — §7's signals) and the base branch (`git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@'`, run from the project root — the finalizer's `<base>`; an empty line means it is resolved again at the skill's step 9) and the lessons store's line count (`wc -l < claudedocs/tickets/_lessons.md` — a number, not the store's content; nothing when there is no store); a `Read` of `03-implementation.md`; a `Read` of `04-review.md`; and a `Read` of the lessons contract, [`lessons-log-fs.md`](../../flow/references/lessons-log-fs.md). The artifacts go through `Read`, never through the Bash print, which carries only small items — the runtime reference's tool-result rule. In the Bash call a file that does not exist prints nothing, and that is its absence — not a §9 error; its commands are separated by `;`, never `&&`, so a missing file cannot stop the rest. A `Read` of an absent `04-review.md` fails, and that failure is its absence. The read carries no lessons-store content: the store's only read is the write-time check (§5).
...
`claudedocs/tickets/_lessons.md`, per [`../../flow/references/lessons-log-fs.md`](../../flow/references/lessons-log-fs.md). The stage reads the store once, when the lesson is drafted: the contract's §4 `Grep` keyword search with its bounded `Read` of the store's last line (offset from the line count §1 printed), issued in the call before the summary write, whose `Edit` then writes the lesson line. A run that captures no lesson never reads it.
...
- **Already closed** — keyed on `06-summary.md` being present with verdict `partial` or `stuck` on its first line, the ticket's own `01-spec.md` `status` being neither `in-progress` nor `in-review`, and `06-summary.md`'s mtime not being older than any of `02-plan.md`, `03-implementation.md`, `04-review.md` and `05-tests.md`. An aborted ticket whose summary sits in `backlog/` matches here, as does an accepted partial in `done/` and a `continue-with-hint` whose implement round has not started. A half-applied `accept-as-partial` — status written, folder not yet moved — also matches; the finalizer's `transition` error already reported what remains.
- **Incomplete tail** — the finalizer never ran, or returned an error — keyed on `06-summary.md` being present while the ticket's own `01-spec.md` `status` is still `in-progress`, and `06-summary.md`'s mtime is not older than any of `02-plan.md`, `03-implementation.md`, `04-review.md` and `05-tests.md`. The signal is the status, never the folder: a finished epic child keeps its folder inside the epic's `tasks/`, and an aborted ticket's summary sits in `backlog/` until a later run moves it back — folder location separates neither from an unfinished tail. `partial-completion` is not a tail signal: implement's Transition 1 resets a hint round to `in-progress` before any code. A summary older than any of those four belongs to an earlier close run that a later plan, implement or review pass has since superseded, so a fresh close owns the ticket. The gate's decisions are conversational state and are deliberately not persisted, so nothing else about the tail is recoverable from the folder.
