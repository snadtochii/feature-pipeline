...
**The preparation read** (the skill's Entry step 4) is one message carrying these tool calls in parallel: one Bash call that prints `claudedocs/tickets/config.yaml`, the frontmatter of `01-spec.md`, `ls -l` over `02-plan.md`, `04-review.md`, `05-tests.md` and `06-summary.md` (presence and mtimes — §7's signals) and `grep '^## ' claudedocs/tickets/_lessons.md` (the store's headings, for §4 of the lessons contract); a `Read` of `03-implementation.md`; a `Read` of `04-review.md`; and a `Read` of the lessons contract, [`lessons-log-fs.md`](../../flow/references/lessons-log-fs.md). The artifacts go through `Read`, never through the Bash print: a Bash result past the runtime's tool-result cap (about 20KB) is persisted to disk with a 2KB preview, and reading it back costs a further call and the same tokens twice. In the Bash call a file that does not exist prints nothing, and that is its absence — not a §9 error; its commands are separated by `;`, never `&&`, so a missing file cannot stop the rest. A `Read` of an absent `04-review.md` fails, and that failure is its absence.
