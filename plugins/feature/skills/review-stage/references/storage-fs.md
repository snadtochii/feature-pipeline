...
**The preparation read** (the skill's Entry step 3) is one message carrying these tool calls in parallel: a `Read` each of `01-spec.md`, `02-plan.md` and `03-implementation.md` (the reviewers' shared base and the handoff's three views come from these copies); and one Bash call that prints `ls -l` over `03-implementation.md` and `04-review.md` (presence and mtimes — §6's signals) followed by the first two lines of `04-review.md` (its label and `fix-step:` marker). The artifacts go through `Read`, never through the Bash print: a Bash result past the runtime's tool-result cap (about 20KB) is persisted to disk with a 2KB preview, and reading it back costs a further call and the same tokens twice. In the Bash call a file that does not exist prints nothing, and that is its absence — not a §7 error; its commands are separated by `;`, never `&&`. Blocker artifacts (§5) are a second message, issued only when `blocked_by` is non-empty.
