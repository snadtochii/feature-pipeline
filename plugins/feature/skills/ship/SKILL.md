...
4. **Parallel gate (`--parallel` only).** If `--parallel [N]` was passed, read [`references/parallel-walk.md`](references/parallel-walk.md) now and follow it: it keeps every PER TICKET hop, brief, verification, and guardrail below, but replaces the one-ticket-at-a-time iteration with ready-set dispatch into isolated worktrees and names the serialization points the orchestrator owns. If the project has no `worktree:` block in `claudedocs/tickets/config.yaml`, log that `--parallel` needs the worktree setup contract and continue with the serial walk below. Without the flag, skip this step — nothing below changes.
