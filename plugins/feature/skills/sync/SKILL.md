...
- **`pr_url` set**: query it live — `gh pr view "<pr_url>" --json number,state,url,mergeCommit,baseRefName` (`gh pr view` accepts a full PR URL; the value is a controlled row field — load it via command substitution, never paste it into a free-text command string). PR state is **never cached** server-side: `gh` is the live source every run, `pr_url` only names *which* PR to ask about.
