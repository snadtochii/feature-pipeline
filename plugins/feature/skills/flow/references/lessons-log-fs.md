...
- Lesson text is free text — handle it with `Read`/`Write`/`Edit`, and search for it with the `Grep` tool, whose pattern is a tool parameter; never interpolate it into shell commands.

## §4 Write-time supersession check

Once the entry is drafted and before appending, search `_lessons.md` once for its subject keywords with the `Grep` tool — case-insensitive, line numbers on, one alternation of the keywords (the concrete path, tool, command, or setting the entry names) with regex metacharacters escaped, so a `.` in a path matches literally; never a shell `grep`, since the keywords are lesson text (§3) — and scan only the matching `^## ` lines (any producer, including `debug/` ones) for one whose single subject matches the new entry's subject. The same message carries a bounded `Read` of the store's last line (`offset` from the store's line count, which the caller printed with its preparation read) — the read `Edit` requires before it touches the file. Those two calls are the check's only read of the store: never a whole-file `Read` or a dump of every heading (§8). No store → create it per §1. No match → append with `Edit`, extending that last line with a newline and the entry. Same subject → **update/merge that one line in place** with `Edit` on the matched line, whose text the search returned, citing all IDs in ascending order per §2's merged-entry form and stamping this newer capture's date, or **skip** when the existing entry already fully covers the point. Because each entry names exactly one subject, the match is reliable. Different or no shared subject → append. No user gate: the check never deletes a distinct fact, only replaces the superseded line — git holds the history. Lines not matching `^## ` are preserved byte-for-byte; never repair or normalize the file.
...
`grep` the file for the matching lines (a single alternation or one pass per keyword), so only the matching atomic entries enter context.
