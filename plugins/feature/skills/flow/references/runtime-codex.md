...
For the review stage's four reviewers, dispatch at most the available slots, wait for that batch, then run the remaining roles with the same shared review input. All four roles must return before findings are merged. The explorer then analyst are sequential; the UI tester, the arbiter and build's post-gate finalizer use the same fresh-spawn operation, each needing a single slot at a point where no reviewer is running.
