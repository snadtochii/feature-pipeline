...
- Post-gate finalizer: retain the git and `gh` mutations its instruction set names (commit, push, open PR), the artifact and ticket writes for PR linkage and the state transition(s), and the worktree removal. It reviews nothing, implements nothing, captures no lesson, asks no question, and delegates to no further child; a condition needing a human returns as its `needs-decision` result for the caller to relay.
...
For the review stage's four reviewers, dispatch at most the available slots, wait for that batch, then run the remaining roles with the same shared review input. All four roles must return before findings are merged. The explorer then analyst are sequential; the UI tester, the arbiter and the close stage's post-gate finalizer use the same fresh-spawn operation, each needing a single slot at a point where no reviewer is running.
