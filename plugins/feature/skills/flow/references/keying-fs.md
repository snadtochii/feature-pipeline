...
- Recency ("current", "newer", "older") compares file mtimes from the same `Glob` over the ticket folder (List artifacts): `06-summary.md` is current when its mtime is not older than any of `03-implementation.md`, `04-review.md` and `05-tests.md` that exist.
