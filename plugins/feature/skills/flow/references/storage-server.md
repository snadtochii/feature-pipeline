...
Two uses degrade instead of stopping, both in `setup` and neither in a stage run: the connector check (Connectivity check, below), where an unanswered `ping` makes server-native unavailable, and an operation whose entry marks it optional, which degrades as that entry states.
...
`ping`, no arguments, read-only — used only by `setup`, before its storage-mode question, to learn whether the connector answers: a non-error result means it does. It is not mode detection, and no stage run calls it.
