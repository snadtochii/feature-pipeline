...
```
Server-native storage operation failed: <operation> (pipeline_<tool>) against the personal server for project <project uuid>. <error detail>. Stopping — fix the server/MCP connection and re-run. Setup: plugins/feature/docs/advanced.md, "Storage mode and the personal server".
```
