...
Every transition is a folder move between state directories (`backlog/`, `in-progress/`, `review/`, `done/`) plus a frontmatter `status` edit — the Transition status operation in [`storage-fs.md`](storage-fs.md). The per-transition mechanics are below.
...
- A child is **materialized** when its spec exists with readable status: `<epic-folder>/tasks/<id>/01-spec.md` exists and has parseable `status` frontmatter (a bare `tasks/<id>/` folder with no readable spec is **not** materialized).
...
A declared child with **no** materialized spec is the **expected** mid-flight state of a just-in-time epic. It fails (C) → `stay`, with no warning — that is normal, not an anomaly.
...
**Descope escape hatch**: a declared child that will never be built must be reflected in the roster, or it blocks the predicate indefinitely (the child blocks (C)). Manual operator remediations: remove the child's ID from `prd.md`'s `children:` (shrinks `declared`), **or** materialize `tasks/<id>/01-spec.md` as a stub with `status: cancelled` (adds it to `materialized` as a terminal child). Either satisfies the predicate. There is no automated roster mutation — descoping is a deliberate human edit.
...
If the ticket is already in progress with the expected status — folder in `in-progress/` — this transition is a no-op (no folder move; the frontmatter overwrite is harmless). This is what lets plan and build both invoke Transition 1 without conflict — whichever runs first triggers the move; the second one is a no-op.
...
A repeat `continue-with-hint` round overwrites `partial-completion` with itself — harmless.
...
- Read the epic's own `status` — `prd.md` frontmatter (it tracks the state-folder location).
- Iterate every child — `<epic-folder>/tasks/*/01-spec.md` — and read each child's `status`.
- Derived states:
