...
- Recency ("current", "newer", "older") compares the artifact rows' `updated_at` from the same `pipeline_list_artifacts` call: `06-summary.md` is current when its `updated_at` is not earlier than that of any of `03-implementation.md`, `04-review.md` and `05-tests.md` that exist.
