...
- **`<STORAGE_MODE>`** is the line `Storage mode: server-native (project <uuid>)`, with the `project` UUID copied verbatim from `claudedocs/tickets/config.yaml` — so the stage's own detection, which reads the same file, agrees with flow's.
