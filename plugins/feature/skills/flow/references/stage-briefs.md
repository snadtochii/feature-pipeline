...
| `<STORAGE_MODE>` | The storage mode the sequencer detected at its start, as a value line: `Storage mode: fs-native` or `Storage mode: server-native (project <id>)`. Authoritative for the stage: it binds its mode from this line and runs no detection of its own, so it can never disagree with the sequencer or detect against a different cwd. Only a standalone invocation — no brief — detects per [storage.md](storage.md). |
