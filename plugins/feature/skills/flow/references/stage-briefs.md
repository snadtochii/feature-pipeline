...
| `<STORAGE_MODE>` | The storage mode the sequencer detected at its start, as a value line: `Storage mode: fs-native` or `Storage mode: server-native (project <id>)`. The stage's own detection runs against the same `config.yaml` and must agree; the line exists so a stage never re-detects against a different cwd. |
