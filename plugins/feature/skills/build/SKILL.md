...
**Entry reads.** The ticket-resolution file, build's storage file and the state-transitions file, each for the detected mode, go out as one parallel message through the runtime's file-read tool, never one shell print of several files. `worktree.md`, `pr-creation.md` and `implementation-handoff.md` are not in it: they are read, as one parallel message, only when State setup's worktree binding needs them ([`runtime-claude.md`](../flow/references/runtime-claude.md) / [`runtime-codex.md`](../flow/references/runtime-codex.md) §Tool results).
...
b. **Validation setup.** Read the project instruction files for **both** runtimes — `CLAUDE.md` and `AGENTS.md` — for the checks the project documents, the current runtime's primary file first (`AGENTS.md` when `$PLUGIN_ROOT` is set and `$CLAUDE_PLUGIN_ROOT` is not, else `CLAUDE.md`), then the other; a file the runtime already loaded as project instructions is taken from context, not re-read. Collect **every** check the project declares — lint, typecheck, and format or tests where they are documented. A heading counts when its text *contains* `Commands`, `Validation` or `Testing`, so `## Validation and extension checklists` matches; inline references to `npm run lint` / `pnpm test` / `cargo check` / `pytest` / etc. count too. A pointer to further documentation is not followed — an undocumented check is simply not collected. Capture the collected set for one chained run per step, after the step's edit message (item 3). If the project documents no commands, log a one-line warning ("No validation commands found in project CLAUDE.md/AGENTS.md — proceeding without skill-body validation") and continue.
...
c. **For each step in `02-plan.md`'s Build Sequence, in order:**
   ...
   2. **Implement the change** following the plan's "Files" and "Pattern to follow" fields, in this message shape. A call joins the previous message unless it needs that message's result — the cost rule is the runtime reference's ([`runtime-claude.md`](../flow/references/runtime-claude.md) / [`runtime-codex.md`](../flow/references/runtime-codex.md) §Tool results), and the tool every content edit goes through is that same reference's §File edits.
      ...
      - **Edit message** — one message carrying every edit, one call per file. Every content edit to a project source file goes through the runtime's structured edit tool ([`runtime-claude.md`](../flow/references/runtime-claude.md) / [`runtime-codex.md`](../flow/references/runtime-codex.md) §File edits) — single-file edits without exception, whatever the session's permission mode allows. Two calls on one file never share a message: the second edit's match text depends on the first's result, so it goes in a following message.

        One carve-out: a **mechanical** change spanning **3 or more files** — a rename, a bulk replace — may be one shell command, and that command prints its match or replacement count, which you compare against the number you expected in the same message, so drift is loud rather than silent. Below three files, or for anything that is not mechanical, the structured tool is the only path. The explicit exclusion is the handoff: the `03-implementation.md` append stays the Bash heredoc chained to validation (item 3).
      ...
      **Step boundary.** The previous step's closing call (item 3's validation-plus-append, or the bare append when no validation is documented) and the next step's plan re-read go in one message, since a read-only plan fetch needs nothing from that call's result. Nothing else crosses a step: step N's edit message and step N+1's read message are never one message.
      ...
      A red validation run is fixed in the same shape — one message of fix edits, one call per file, through the same tool.
   3. **Run validation and record the step in one call.** The checks §1b collected run as **one `&&`-chained command**, **once per plan step**, with a formatting check first when one is documented, and this step's `## Steps` entry chained onto that same command as a heredoc append to `03-implementation.md` — what changed and where, constraints discovered, rough edges left on purpose, validation state, per [`references/implementation-handoff.md`](references/implementation-handoff.md) §2 and §5, which owns the chain's shape. No check runs again on its own afterwards — a formatter that rewrites a file does not earn a second typecheck message. Never spend a turn of its own on the entry. A red run appends nothing: fix the errors, then re-issue **the same chain**. With no validation commands documented, the bare append is the step's closing call and crosses the step boundary (item 2; handoff §5).
...
d. **After all plan steps are implemented**, close the phase in one call, its last action: the checks §1b collected run once across all changes, and the completion of `03-implementation.md` is chained onto that same command — one run of the full documented set, and nothing else in the phase end runs any of it again. A red run appends nothing: fix the cross-cutting failures in-context, then re-issue the same chain. The append carries a `(revisit)` `## Steps` entry for any step those fixes changed, followed by the `## Rationale` section — one entry per step, why this shape and what was rejected (§3 and §5 of the handoff reference). The implement phase ends here. Print one line — `Implement phase complete for <ticket-id> — handoff: <absolute path of 03-implementation.md>`. With `--implement-only`, end. Standalone, continue into the stage chain (step 3) at the review stage.
