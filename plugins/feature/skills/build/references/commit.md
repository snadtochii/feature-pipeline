...
**Working tree**: when the caller has a worktree bound, every `git` command below runs as `git -C "<wt-path>" …` per [`worktree.md`](worktree.md) §3, which explains why `-C` is load-bearing here rather than cosmetic. §1 additionally excludes every path on the run's exclusion list — the copied files `worktree.md` §2 step 4 found **not ignored in the worktree** — via `git reset -q -- "<rel-path>"` alongside the `claudedocs` and session-state exclusions. The performer is responsible for that list being current: it is re-derived from the tree at commit time, never read back from the `## Worktree` record (worktree.md §2 step 1), so a resumed run commits with the same protection as the run that provisioned. The message file passed to `git commit -F` is a `/tmp` or scratchpad path and is unaffected.
