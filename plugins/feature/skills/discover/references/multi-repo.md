...
Canonical logic for discover's multi-repo handling. Read when Phase 1's workspace-shape detection lands on multi-repo — a single-repo workspace never needs this file. Referenced by `discover` (which writes the field) and by `ship`'s parallel walk (`skills/ship/references/parallel-walk.md` §1, which consumes it for repo-lane partitioning).
