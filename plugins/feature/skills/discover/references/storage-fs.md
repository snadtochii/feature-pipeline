...
2. If not:
   ...
   - Print one line: `Run /feature:setup for validation commands, test and worktree config.`
3. If `claudedocs/tickets/` already exists, read the prefix from `claudedocs/tickets/config.yaml` (parse the YAML and extract the `prefix` field)
   - If `config.yaml` is missing, scan existing ticket filenames to infer the prefix, or ask the user. Once known, write `config.yaml` so the next run doesn't repeat the inference, then print the same one line: `Run /feature:setup for validation commands, test and worktree config.`
