...
**What you see.** A subagent's output surfaces when it returns, so there is no live checkpoint stream during build. Flow prints plan's "Plan Saved" summary when plan returns, a one-line pointer when build starts (`03-implementation.md` is updated per plan step — the live view), and build's verdict line and summary when build returns.
