...
A `flow` run loads in five contexts: the conversation that invoked `flow`, then one stage subagent each for plan, implement (`build --implement-only`), `review-stage` and `close-stage`. Each stage's files land in its own context, so the peak context carries one stage's load, never plan's or flow's on top of it. The review work and `confidence-scale.md` load in the `review-stage` context; the test checkpoint, the verdict gate and their references (`skip-artifacts.md`, `test-preflight.md`) load in the `close-stage` context — none of them in the implement context. The shared references (`storage.md`, `storage-<mode>.md`, `ticket-resolution-<mode>.md`) are loaded once per context, five times per run, so total input tokens rise with each stage while the peak falls. No word count of the current per-stage file sets is recorded here.
