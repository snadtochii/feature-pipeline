...
By default a passing build stops at the close stage's verdict gate and asks whether to commit (configurable — see [Commit behavior](#commit-behavior)). With `--pr`, the close stage ships non-interactively instead: once the gate resolves, a finalizer child detects the base branch, creates a branch (forking from `main` when needed), commits, pushes to `origin`, opens a **GitHub pull request** via `gh`, and lands the ticket in a `review/` state (`status: in-review`) rather than `done/`. You're notified with the PR URL.

Once the PR merges, re-run `/feature:flow <id>` (or `/feature:close-stage <id>`) — the close stage detects the merge and finalizes the ticket to `done/`. To finalize merged reviews in batch or unattended, run `/feature:sync`: it scans every ticket in `backlog/`, `in-progress/`, and `review/` and promotes the merged ones to `done/` in one pass. In epic mode, `--pr` opens one PR per child.

`--pr` needs the GitHub CLI (`gh`) installed and authenticated and a GitHub `origin` remote. If any is missing, the close stage degrades gracefully — its finalizer commits locally, finalizes to `done/`, and prints one line explaining why the PR step was skipped. It never blocks the verdict gate.

## Required UI checks

Every browser pass — the close stage's test checkpoint and ship's `--ui-test` pass — runs the acceptance criteria plus a fixed set of required checks, defined once in `skills/build/references/ui-checks.md` and injected into the `ui-tester`'s brief:
...
- **Findings, not polish** — overflow or horizontal scroll, overlapping or clipped controls, an unreadable or truncated label, and layout shift when an error appears are failures of the UI-states criterion. They land under `## Failed Criteria` in `05-tests.md` and affect the close stage's verdict like any other failed criterion. A wrapped heading or a spacing preference stays an observation.
...
On a passing non-`--pr` build, the close stage's verdict-gate commit question is governed by the [`git.commit` config default](#commit-behavior) — ask, always commit, or never commit. `--no-commit` is the per-run override on top of that: pass it to leave this run's changes uncommitted with no prompt, whatever the config says. The ticket still finalizes to `done/` — the commit decision affects git only, never the folder transition — and the final message points at `git status`.

`--no-commit` contradicts `--pr` (an explicit "ship it", which must commit and push): passing both stops the run at start with a one-line error — flow's SETUP, standalone build's flag validation, or the close stage's own. The flag propagates `flow → close stage` and, in epic mode, is forwarded to every child.

## Worktree isolation (`--worktree`)

Running several tickets at once means several sessions competing for one working tree. `--worktree` gives a build its own: before implementing, it fetches, pre-creates the ticket branch `<type>/<ID>-<slug>` from `origin/<base>`, adds a worktree at `../<repo-dirname>-worktrees/<TICKET-ID>` (a sibling of the repo, never inside it), copies the [`.worktreeinclude`](#worktree-setup) files, and runs [`worktree.setup`](#worktree-setup). Every git and project command for the rest of the run is aimed at that worktree; the ticket folder, state transitions, and the lessons log stay in the main checkout.

The finalizer that the close stage spawns **removes** the worktree once the work is committed on the branch or pushed — commits are repository-level, so the worktree then holds nothing the repository does not. It is **left in place**, with its path printed, when the work is uncommitted (`--no-commit`, or `git.commit: never`) or the verdict is `partial`/`stuck` — uncommitted work is never silently orphaned. A resumed build re-attaches to a worktree recorded by a prior run whether or not you pass the flag again.

`--worktree` contradicts nothing; it composes with `--pr`, `--no-commit`, and `--no-ui-testing`. It propagates `flow → implement stage` (`build`) and, in epic mode, is forwarded per child; the review and close stages re-bind the recorded worktree themselves. Four cases degrade to an ordinary in-place build with a one-line notice:

- The ticket's `repos:` names **2+ repos** — one worktree cannot span repositories. Exactly one entry is supported.
- A `blocked_by` **blocker's code is not on the base** yet (it finished without `--pr`, so its commits sit on a local branch) — a fresh worktree would not contain it. If the current checkout does not carry it either, the notice says so plainly: building in place does not conjure the code, and you need to merge or check out that branch first.
- On `ship`, combined with **`--parallel`**, which already provisions a worktree per dispatched ticket.
- On `ship`, whenever the run's base is **not the repository's default branch** — an epic chain's `integration/<epic-id>`, or any run under `--base <non-default>`. Neither `flow` nor `build` takes a base argument, so build would resolve `origin/HEAD` and fork the worktree from the default trunk while the PR targets something else. Use `--parallel` for isolation on a chain.

A missing [`worktree:` config block](#worktree-setup) is **not** a degrade: the worktree is still created and only the setup step is skipped, with a notice — the flag is an explicit isolation request, and dependency install is the only thing you then do by hand.

**Cost.** `worktree.setup` runs once per worktree, and a fresh worktree means a fresh dependency tree — typically the flag's dominant cost in both wall-clock and disk. `flow --worktree <EPIC>` forwards per child and tears each worktree down as it finishes, so an N-child epic pays N installs back to back with no reuse. Worth pointing `worktree.setup` at a shared store (`pnpm install` against a global store, a warm package cache) before using the flag per epic child. Worktrees left in place by the endings above accumulate until you remove them.

## Stage subagents and per-stage models (`--plan-model`, `--build-model`)

`flow` runs each stage in its own subagent: plan, implement (`build`), review (`review-stage`) and close (`close-stage`), in that order. Each starts from a fresh context whose inputs are the artifacts the earlier stages saved — `02-plan.md`, then the implementer handoff `03-implementation.md`, then `04-review.md` — so none of plan's exploration, implement's edits, the reviewer reports or the test output ever accumulate in the conversation that invoked `flow`, or in a later stage. Flow's context holds resolution, validation, the four spawns, and what the stages return. Standalone `/feature:plan` runs in your conversation; standalone `/feature:build` runs the implement phase in your conversation and then spawns the review and close stages itself, from the same briefs flow uses.

**Runtime selection.** `flow`, `plan`, `build`, `review-stage`, `close-stage`, and `ship` select their execution runtime from the active tools using [`runtime.md`](../skills/flow/references/runtime.md), independently of ticket storage. Claude uses native `Skill`, `Agent` (or its `Task` alias), `SendMessage`, and registered role definitions. Codex reads the same skill files in the current agent, creates fresh children with `fork_turns: "none"` where supported, and loads the shared agent bodies with explicit operational boundaries. Each child receives the absolute plugin root and selected runtime path, so nested calls use the same installation. These instructions preserve role boundaries; they do not claim native Codex tool filtering.

**What you see.** A subagent's output surfaces when it returns, so there is no live stream inside a stage. Flow prints plan's "Plan Saved" summary when plan returns, a one-line pointer when implement starts (`03-implementation.md` gets an entry per plan step — the live view), and each later stage's report as it returns: implement's result, review's result block, and close's verdict line and summary.

**Decisions inside a stage.** The close stage's verdict gate (the commit question; `accept-as-partial | continue-with-hint | abort` on a `partial`/`stuck`), a branch-safety choice its finalizer hands back, and plan's two auto-mode stops (no-default open questions, a complexity overflow) still reach you: the stage pauses with the block, flow prints it and waits, and your answer resumes the same subagent (Codex uses `followup_task` to start its next turn). Implement and review never pause. `continue-with-hint` ends the close stage with your hint, and flow spawns a fresh implement stage carrying it, followed by review and close again. On a surface where a finished subagent cannot be resumed, flow says so in one line and re-spawns the stage with your answer supplied up front (close resumes the recorded pending operation from available artifacts; plan re-runs its exploration). Under `ship`, the implementer's brief answers instead of a human.

**Per-stage models.** `--plan-model <alias|inherit>` and `--build-model <alias|inherit>` (default `inherit`) bind the model of the plan spawn and of the implement, review and close spawns respectively:

- **Claude Code** — the value is passed verbatim as the spawn's `model` argument: a family alias (`sonnet`, `opus`, `haiku`, `fable`) or a full model ID; aliases are preferred so the plugin tracks your current family. `inherit` runs the stage on the model your session runs on.
- **Codex** — supply a model ID accepted by the active spawn surface. The value is passed unchanged when an override is exposed; when it is hidden, the stage inherits and flow prints one line saying so. Fresh context is retained for both inheritance and overrides. Claude aliases are never translated to OpenAI models.
- **Fallback** — a value the runtime rejects (unknown alias, model unavailable to the account) falls back to inheritance after one retry, with one printed line. A model override never changes a verdict and never stops a run.
- The flags bind the spawn only: nothing reaches any stage's own arguments, and the review stage's reviewers and the close stage's `ui-tester` and finalizer keep their registered agent models on Claude; Codex role children inherit their runtime model without applying Claude frontmatter. Model names live on the command line — never in `claudedocs/tickets/config.yaml`. A flag whose stage the run skipped (resumption) prints an unused notice rather than silently doing nothing. In epic mode both flags are forwarded to every child.

**Depth and capacity.** `flow → stage → role` requires two child layers below the caller; `ship → worker → stage → role` requires three. The runtime checks its active nesting and concurrency limits; unavailable nested spawning is reported as a capability error with artifacts preserved. The review stage runs all four reviewer roles in bounded batches when they cannot run concurrently; the close stage's `ui-tester` and its post-gate finalizer each take a single slot, one after the other. Ship reserves a stage and at least one leaf slot for each worker and reduces its effective `--parallel` count when needed. With four shared slots including the root, one ship worker runs at a time and its leaf roles run serially. The requested worker count is a ceiling, not a guarantee. Ship's `--parallel` workers hand their state clause, workdir, and base branch to the stages through a `## Stage overrides` section that flow copies verbatim into each stage brief.
...
test:
  ...
  start_timeout: 60                   # seconds to wait for start to respond (default 60, max 540)
  auth:
...
A dated measurement of plugin version 3.5.4, the release that completed the per-mode split: what each skill read on an ordinary run — its `SKILL.md` plus every reference it loaded unconditionally in that mode — before and after the split (`wc -w`; "before" is the commit preceding the split, `b8a336a`). At 3.5.4 build ran implement, review and test in one context, so its row covers all three; the figures describe that version, not the current file set. Before the split no per-mode file existed, so both "before" columns count the same files and are identical except for `discover`, whose server column already had a server-only reference.
...
Counted per row at 3.5.4: the skill's `SKILL.md`; the `storage.md` detection stub; the flow `storage-<mode>.md` operation vocabulary (every skill reaches it — directly, or through the "operations named below are defined in …" header of its skill-local pair); the skill-local `storage-<mode>.md` where one exists (build, sync, ship, discover); the shared flow pairs the skill dispatches to (plan/build: ticket-resolution, state-transitions, lessons-log; flow: ticket-resolution, keying; sync: state-transitions; ship: lessons-log; debug: ticket-resolution, lessons-log); and the mode-neutral references read on every run (build: `confidence-scale.md`, `stuck-detection.md`; sync: `gh-preconditions.md`, `pr-creation.md`; ship: `reviewer-prompt.md`; flow: `stage-briefs.md`, counted in the sentence below the table, not in the row). Gating assumed: `debug` with a ticket in scope, `sync`'s no-argument scan; discover's `templates/` are inputs, not references, and are excluded. Flag- or shape-gated references are not in the figure: `worktree.md` (3501 → 3373), `pr-creation.md` for build (2367 → 2239), `parallel-walk.md` (3902 → 3593), `ui-verification.md` (1227 → 1075), `epic-walk.md` (1282 → 1046 fs / 1114 server), `multi-sibling.md` (1107 → 478 — its fs generation steps moved into discover's fs file), and the unchanged `commit.md`, `test-preflight.md`, `validation-hook.md`, `multi-repo.md`, `exploration-mode.md`. Ship's pair is loaded once at SETUP and carries sections a serial, non-`--ui-test` run never uses — the parallel-walk state forms and the UI evidence home (≈220 fs / ≈205 server words) plus the `--parallel` half of the ticket-argument block — which is why its server figure does not drop; one pair with one load was kept over a second pair, whose header and section scaffolding would cost more than those sections.

`flow` at 3.6.0, measured over the row's file set plus the mode-neutral `stage-briefs.md` it reads at STAGE EXECUTION: 7803 words fs-native / 8433 server-native (`stage-briefs.md` ≈ 2.2k of the difference from the table's row; the rest is `SKILL.md` growth and the `keying-<mode>.md` §5).

A `flow` run loads in five contexts: the conversation that invoked `flow`, then one stage subagent each for plan, implement (`build --implement-only`), `review-stage` and `close-stage`. Each stage's files land in its own context, so the peak context carries one stage's load, never plan's or flow's on top of it. The review work and `confidence-scale.md` load in the `review-stage` context; the test checkpoint, the verdict gate and their reference (`test-preflight.md`) load in the `close-stage` context — none of them in the implement context. The shared references (`storage.md`, `storage-<mode>.md`, `ticket-resolution-<mode>.md`) are loaded once per context, five times per run, so total input tokens rise with each stage while the peak falls. No word count of the current per-stage file sets is recorded here.
