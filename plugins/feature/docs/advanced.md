...
`flow` today, measured at the stage-subagent change over the row's file set plus the mode-neutral `stage-briefs.md` it reads at STAGE EXECUTION: 7803 words fs-native / 8433 server-native (`stage-briefs.md` ≈ 2.2k of the difference from the table's row; the rest is `SKILL.md` growth and the `keying-<mode>.md` §5 added with it, on top of prose that later moved into the keying pair). The plan and build figures above are unchanged in size — what changed is where they land: in each stage's own subagent context, not in the conversation that invoked `flow`. Two consequences for a whole run: build's peak context no longer carries plan's load or flow's, and the shared references (`storage.md`, `storage-<mode>.md`, `ticket-resolution-<mode>.md`) are loaded once per context — three times per run instead of once — so total input tokens rise while the peak falls.
#### Claude Code setup
