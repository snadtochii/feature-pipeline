...
By default a passing build stops at the verdict gate and asks whether to commit (configurable — see [Commit behavior](#commit-behavior)). With `--pr`, build ships non-interactively instead: once the gate resolves, a finalizer child detects the base branch, creates a branch (forking from `main` when needed), commits, pushes to `origin`, opens a **GitHub pull request** via `gh`, and lands the ticket in a `review/` state (`status: in-review`) rather than `done/`. You're notified with the PR URL.
...
`flow` runs each stage in its own subagent. Plan runs first; its only output is `02-plan.md`. Build then starts from a fresh context whose inputs are the spec and that plan — none of plan's exploration or reasoning, and none of build's own edits, reviewer reports, or test output ever accumulate in the conversation that invoked `flow`. Flow's context holds resolution, validation, the two spawns, and what the stages return. Standalone `/feature:plan` and `/feature:build` are unchanged: they run in your conversation.

**Runtime selection.** `flow`, `plan`, `build`, and `ship` select their execution runtime from the active tools using [`runtime.md`](../skills/flow/references/runtime.md), independently of ticket storage. Claude uses native `Skill`, `Agent` (or its `Task` alias), `SendMessage`, and registered role definitions. Codex reads the same skill files in the current agent, creates fresh children with `fork_turns: "none"` where supported, and loads the shared agent bodies with explicit operational boundaries. Each child receives the absolute plugin root and selected runtime path, so nested calls use the same installation. These instructions preserve role boundaries; they do not claim native Codex tool filtering.

**What you see.** A subagent's output surfaces when it returns, so there is no live checkpoint stream during build. Flow prints plan's "Plan Saved" summary when plan returns, a one-line pointer when build starts (`03-implementation.md` gets a `## Steps` entry per plan step — the live view), and build's verdict line and summary when build returns.

**Decisions inside a stage.** Build's verdict gate (the commit question; `accept-as-partial | continue-with-hint | abort` on a `partial`/`stuck`), a branch-safety choice its finalizer hands back, and plan's two auto-mode stops (no-default open questions, a complexity overflow) still reach you: the stage pauses with the block, flow prints it and waits, and your answer resumes the same subagent (Codex uses `followup_task` to start its next turn) — `continue-with-hint` keeps looping inside build with its context intact. On a surface where a finished subagent cannot be resumed, flow says so in one line and re-spawns the stage with your answer supplied up front (build resumes the recorded pending operation from available artifacts; plan re-runs its exploration). Under `ship`, the implementer's brief answers instead of a human.

**Per-stage models.** `--plan-model <alias|inherit>` and `--build-model <alias|inherit>` (default `inherit`) bind the model of the plan and build spawns respectively:
