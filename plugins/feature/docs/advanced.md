...
- **Codex** — the value is the spawn's model override when the active surface exposes one; when it is hidden, the stage inherits and flow prints one line saying so.
...
- The flags bind the spawn only: nothing reaches `plan`'s or `build`'s own arguments, and build's reviewers and `ui-tester` keep the models their agent files declare. Model names live on the command line — never in `claudedocs/tickets/config.yaml`. A flag whose stage the run skipped (resumption) prints an unused notice rather than silently doing nothing. In epic mode both flags are forwarded to every child.

**Nesting depth.** Subagents may spawn subagents to a depth of 5 (Claude Code v2.1.172 and later). A bare `flow` run is you (1) → stage subagent (2) → the stage's own subagents (3): plan's explorer and analyst, build's four reviewers and `ui-tester`. Under `ship`, including `ship --parallel`, the tree is orchestrator (1) → implementer (2) → stage subagent (3) → the stage's own subagents (4) — one level of headroom. Ship's `--parallel` workers hand their state clause, workdir, and base branch to the stages through a `## Stage overrides` section that flow copies verbatim into each stage brief.
