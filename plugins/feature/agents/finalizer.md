...
3. **Push and open the PR** (when `--pr` is in the instruction set). Run the preconditions; on failure degrade to the local commit and report the degradation reason. Otherwise push, open the PR, capture its URL. With screenshots named, probe for `--attach` first and open the PR from the attach posted body with the listed `--attach` arguments; a failed probe opens it from the manifest variant, and a non-zero attach exit is first reconciled against the PR it may have created (stdout URL, or an open PR for the branch) — a created PR is the success result, never re-created — and retried once from the manifest variant only when no PR exists, both reported as the `Screenshots:` outcome line in `notes`. An already-pushed branch or an already-open PR is reported, not duplicated — an open PR is never re-created, so its screenshots are never uploaded twice. When the branch decision forks from the base, the uncommitted work travels by `git stash -u` and `git stash pop` exactly as the PR reference writes them — never `stash apply` plus a drop by ref, which leaves the entry behind for the caller to clean up.
