...
2.5. Auth check: if the URL routes to a login page, authenticate before
     testing. The spawning skill (close-stage, or ship's --ui-test pass) may have injected an auth recipe into this
     ...
     The recipe is injected by the spawning skill (single source of truth) — don't read
