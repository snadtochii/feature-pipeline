...
implement stage's pre-spawn fence probe ([stage-4-implement.md](stage-4-implement.md) §2) and in
the spec-mover's moves and deletions; §3's closed path classes are what keep them data. Nothing
...
> `paths.inventory` — source changes, spec moves and deletions, and `CONTEXT.md` and ADR edits
> all count; a binary file counts zero.
