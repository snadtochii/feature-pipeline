...
`claudedocs/tickets/_lessons.md` is a project-local log of gotchas captured at the build verdict gate. Each entry is a `^## ` header line that is **atomic and date-stamped** — exactly one subject (one concrete path, tool, command, or setting) per line — in the form `## <ticket-id> (<verdict>, <YYYY-MM-DD>): <one atomic lesson>` (a line may cite multiple IDs once entries are merged; the date uses the environment's current date at capture). Keep each line short enough to read at a glance — a soft guideline, not a hard limit; a lesson that needs more room spans more than one subject, so it splits into multiple atomic lines or graduates to `CLAUDE.md`. Build captures with a **write-time supersession check**: because each entry names a single subject, a new lesson on the same subject as an existing entry reliably updates/merges that one line (or is skipped) instead of appending — no user gate; git holds the history. On a same-subject conflict, **prefer-newest** wins: the newer information replaces the stale line and the date reflects the newer capture. A same-subject second capture is the **promotion trigger**: build proposes moving the lesson into the project's `CLAUDE.md` (`## Lessons` section, created if missing), showing the exact line and target before the edit is applied — never a bare yes/no; unattended runs skip the proposal and keep the log line (it re-proposes at the next interactive capture) — and, on accept, removes it from the log — durable truths graduate upward and the file stays small. A lesson that can't be stated atomically as one subject on one line is proposed as a CLAUDE.md edit instead of appended. The standalone `debug` skill is a second producer: on a root cause that's a project-specific would-recur gotcha, it appends a line in the same `^## `-matching format — `## <ticket-id> (debug, <YYYY-MM-DD>): …` with a ticket in scope, or `## debug/<slug> (<exit>, <YYYY-MM-DD>): …` standalone — without running the supersession check (build's next capture absorbs any same-subject duplicate). **Consumption is grep-scoped and on-demand.** A consumer (plan's Phase 1; ship's per-ticket brief and reviewer prompt) derives subject keywords from the ticket it is working — the concrete paths, tools, commands, and areas in scope — `grep`s `_lessons.md` for the matching atomic entries, and loads only those lines; it never reads the whole file. Because each entry names a single subject, a subject keyword selects the lines whose gotcha is about that subject. Plan additionally caps its selection at 5 ticket-relevant entries and threads them into the requirements-analyst's open-questions surface. No file, or no match → the consumer carries no lessons block.
