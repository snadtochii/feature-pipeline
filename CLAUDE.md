...
`claudedocs/tickets/_lessons.md` is a project-local log of gotchas captured at the build verdict gate. One header line per ticket: `## <ticket-id> (<verdict>): <one-sentence lesson>` — one sentence, one topic, ≤200 characters (a line may cite multiple IDs once entries are merged). Build captures with a **write-time supersession check**: a new lesson on the same named subject as an existing entry updates/merges that one line (or is skipped) instead of appending — no user gate; git holds the history. A same-topic second capture is the **promotion trigger**: build proposes moving the lesson into the project's `CLAUDE.md` (`## Lessons` section, created if missing), showing the exact line and target before the edit is applied — never a bare yes/no; unattended runs skip the proposal and keep the log line (it re-proposes at the next interactive capture) — and, on accept, removes it from the log — durable truths graduate upward and the file stays small. A lesson that can't fit one sentence is proposed as a CLAUDE.md edit instead of appended. The standalone `debug` skill is a second producer: on a root cause that's a project-specific would-recur gotcha, it appends a line in the same `^## `-matching format — `## <ticket-id> (debug): …` with a ticket in scope, or `## debug/<slug> (<exit>): …` standalone — without running the supersession check (build's next capture absorbs any same-topic duplicate). Plan's Phase 1 selects at most 5 ticket-relevant entries — never the whole file — and threads them into the requirements-analyst's open-questions surface.
