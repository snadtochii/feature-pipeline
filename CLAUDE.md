...
- Only `plugins/feature/skills/`, `plugins/tidy-loop/` and `plugins/deepen/` are link-checked — those are `scripts/check-md-links.sh`'s three roots, and the other five validators are `plugins/`-scoped. A relative link or `#anchor` added under `docs/`, `plugins/feature/docs/` (incl. `advanced.md`) or the root `README.md` is checked by nothing; resolve it by hand before the PR.
