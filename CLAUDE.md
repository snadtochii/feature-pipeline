...
10. **Mode-split check** — run `scripts/check-mode-split.sh`; it must exit 0. It is the executable form of the per-mode reference convention (no other-mode token in a `-fs`/`-server` file) and the only guard against a dangling relative `.md` link anywhere under `plugins/feature/skills` — the link check covers every reference, not just the mode pairs.
