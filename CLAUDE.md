...
- **Forwarding `--no-ui-testing` through `ship`** (`skills/ship/`). The flag is wired `flow → build` only. `ship` runs fully autonomously and auto-merges with no per-ticket human gate, yet `--no-ui-testing` defers browser verification *to* a human at PR review — so making `ship` forward it would silently auto-merge UI changes that were never browser-verified. Threading the flag through `ship` needs its own autonomy-vs-verification decision (e.g. an explicit "UI not browser-verified" note in `ship`'s merge report) before it's safe; deferred until that's designed.
