...
- **`flow`** orchestrates `plan → build` with the completion gate. `plan` runs non-interactively under flow (flow passes the internal `--auto` signal), so build's verdict gate is the only gate; run standalone, `plan` uses interactive plan mode (its own gate). `plan` includes Phase 1 pre-plan synthesis (codebase exploration + open-questions surfacing) before plan design. Flag surface is `--pr`, `--no-commit`, and `--no-ui-testing` (the flow→plan `--auto` signal is internal wiring, not a user-facing flow flag; all three are propagated to build); resumption is auto-detected from on-disk artifacts (users delete artifacts to start fresh).
