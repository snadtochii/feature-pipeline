...
- **`flow`** orchestrates `plan → build` with the completion gate. Each stage runs as its own **stage subagent** (spawned through the selected runtime from the templates in `skills/flow/references/stage-briefs.md`): flow's context holds only resolution, validation, the two spawns, and the relayed summaries; the plan→build handoff is `02-plan.md` on disk. A user-facing stop inside a stage (plan's no-default questions or complexity overflow, build's verdict gate) pauses the stage subagent — flow relays the block to its caller — attended, it ends its turn and waits, never choosing itself — and resumes the **same** agent with the choice (the selected runtime's resume operation; a surface that cannot resume gets one printed re-spawn with the answer supplied), so `continue-with-hint` keeps looping inside build. `plan` runs non-interactively under flow (the plan brief passes the internal `--auto` signal), so build's verdict gate is the only gate; run standalone, `plan` uses interactive plan mode (its own gate). `plan` includes Phase 1 pre-plan synthesis (codebase exploration + open-questions surfacing) before plan design. Flag surface is `--pr`, `--no-commit`, `--no-ui-testing`, `--worktree`, and `--hint` (propagated into the build brief; `--auto` is internal wiring, not a user-facing flow flag), plus `--plan-model` / `--build-model` (default `inherit`; bind the stage spawn's `model` argument, never a stage's args, never `config.yaml`); resumption is auto-detected from on-disk artifacts (users delete artifacts to start fresh).
...
Centralized cross-stage rules live in `skills/flow/references/`, one file per storage concern **per storage mode**: `storage.md` is the mode-detection stub, and every other concern is a `<concern>-fs.md` / `<concern>-server.md` pair of which a run reads exactly one — the file for the mode detected once at skill start. The folder also holds two flow-private pairs — `epic-walk-fs.md` / `epic-walk-server.md` (the epic walker) and `keying-fs.md` / `keying-server.md` (how flow reads its routing signals, `kind`, the start-fresh deletion, and — §5 — the mode-valued stage-brief placeholders; build's server file points at its §1 for the shared epic-child `in-review` divergence) — plus one mode-neutral flow-private file, `stage-briefs.md` (the plan and build stage-subagent brief templates, the spawn contract and model-override mapping, the pause/resume protocol for user-facing stops, the verbatim forwarding rule for instructions flow itself received, and the relay/failure rules; it names the two mode-valued placeholders and takes their values from `keying-<mode>.md` §5); only the cross-stage pairs are summarized here. Skill-local pairs follow the same convention: `skills/build/references/storage-fs.md` / `storage-server.md` holds build's own storage mechanics (working copy, per-write verdict rule, resumption keying, PR linkage, the worktree config-presence rule), loaded once at build's State setup and cited by `build` and by its `worktree.md` / `pr-creation.md` references on behalf of whichever skill runs them. The standalone skills carry the same shape: `skills/sync/references/storage-fs.md` / `storage-server.md` (scan set, PR lookup key, Transition 6 mechanics, report shaping), `skills/ship/references/storage-fs.md` / `storage-server.md` (metadata and roster, lessons store, the ground-truth blocks inlined into subagent briefs, PR linkage, in-review evidence, parallel-walk state forms, UI evidence home — cited by `ship` and its `parallel-walk.md` / `reviewer-prompt.md` / `ui-verification.md`), and `skills/discover/references/storage-fs.md` / `storage-server.md` (Phase 0 infrastructure, ID allocation, single- and multi-mode generation, partial-failure honesty — cited by `discover` and its `multi-sibling.md`). `plan` and `debug` dispatch to the shared flow pairs and keep no pair of their own.
...
- **Commit-behavior config** lives in the same `claudedocs/tickets/config.yaml` under an optional `git:` block — presets the verdict gate's commit question on a passing non-`--pr` build. Like `test:`, it is **model-read** by the build skill, never by `hooks/validate.sh`:
  - `git.commit` — `prompt | always | never`. `prompt` (or absent block/key) is the interactive ask; `always` skips the prompt and commits onto the current branch per `skills/build/references/commit.md` (no push, no PR); `never` skips the prompt and leaves the changes uncommitted (the ticket still finalizes). An unrecognized value degrades to `prompt` with a one-line notice.
  - Per-run overrides beat the config: `--no-commit` forces "leave uncommitted"; `--pr` commits/pushes/opens the PR over `never` with a one-line notice. `--pr` + `--no-commit` together is a start-of-build contradiction error. Bound at build's State setup — see `skills/build/SKILL.md`.
...
10. **Mode-split check** — run `scripts/check-mode-split.sh`; it must exit 0. It is the executable form of the per-mode reference convention (no other-mode token in a `-fs`/`-server` file, no half pair) and the only guard against a dangling relative `.md` link anywhere under `plugins/feature/skills` or `plugins/tidy-loop` — the link check covers every reference in both trees' committed markdown, not just the mode pairs; vendored and generated directories are pruned from the walk.
