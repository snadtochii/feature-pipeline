...
|---|---|
| `flow` (thin sequencer) | Read, Glob, Grep, TodoWrite, Skill, Task, SendMessage + pipeline MCP tools (`pipeline_get_ticket`, `pipeline_list_tickets` for the epic walk's derived roster, `pipeline_get_artifact`, `pipeline_list_artifacts` for resumption keying, `pipeline_delete_artifact` for SETUP's downstream-artifact invalidation — the one skill-side artifact deletion) — `Task` spawns the plan and build stage subagents from `skills/flow/references/stage-briefs.md`, `SendMessage` resumes a stage paused at a user-facing stop with the relayed choice; flow keeps no Write/Edit/Bash — MCP reads + the invalidation delete fit its thin-sequencer budget |
