...
- **Storage mode** lives in the same `claudedocs/tickets/config.yaml` as optional top-level `mode` and `project` keys — the fs-native ↔ server-native switch, model-read at skill start per `skills/flow/references/storage.md`:
  ...
  - `config.yaml` itself stays fs-local in both modes — the file is project execution config plus the mode marker, not ticket data. `prefix` remains meaningful only for fs allocation (server IDs come from the registry-configured prefix).
- **Execution config** — the optional `test:`, `git:`, and `worktree:` blocks live in the same `config.yaml`; keys and trust rules are in `docs/advanced.md`. All of them are model-read. Validation commands are not config: build collects them from the project's `CLAUDE.md`/`AGENTS.md` (`skills/build/references/validation-chain.md`).
