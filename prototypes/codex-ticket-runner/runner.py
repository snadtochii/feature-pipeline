...
def run_check(path, state, name, final=False, rerun=False):
    ...
    folder.mkdir(parents=True, exist_ok=True)
    # Preserve the log an earlier reviewer saw, even when this check is rerun.
    log = folder / (name + "-" + uuid.uuid4().hex + ".log")
    ...
    results[name] = {"tree": tree, "exit_code": code if tree == after else -1,
                     "argv": check["argv"], "timeout": check.get("timeout", 120),
                     "log": str(log), "log_digest": digest(log)}
    return results[name]
