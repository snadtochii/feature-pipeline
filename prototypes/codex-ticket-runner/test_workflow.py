...
class WorkflowTests(RunnerFixture):
    ...
    def test_rerun_preserves_reviewed_log_and_requires_new_review(self):
        self.ready()
        record = json.loads(self.state.read_text())["tickets"]["DEMO-1"]
        from pathlib import Path
        old_log = Path(record["checks"]["unit"]["log"])
        old_bytes = old_log.read_bytes()
        result = self.cli("check", "unit", "--rerun")
        self.assertNotEqual(str(old_log), result["log"])
        self.assertEqual(old_log.read_bytes(), old_bytes)
        self.assertEqual(self.cli("status")["outcome"], "review")
        self.cli("review", str(self.root / "review.json"), ok=False)
