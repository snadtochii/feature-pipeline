...
a. The Epic-completion predicate inside the **last child's** build verdict gate (Transition 2's epic variant in [`state-transitions.md`](state-transitions.md)) already moved the epic subtree from `in-progress/<EPIC>/` to `done/<EPIC>/` and updated `prd.md`'s `status` to `done` — in epic-mode the walker runs every declared child, so by the final child the predicate's roster check is satisfied. Flow does NOT repeat this — it has already happened.
...
- **`prd.md` malformed or missing**: defer to [`ticket-resolution.md`](ticket-resolution.md)'s error handling.
