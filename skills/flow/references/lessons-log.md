...
- The file is **project-local context** — constraints and gotchas that bit a prior ticket and would bite the next one if not surfaced. Generic best practices don't belong here (§3).
...
**Never read the whole file into context.** A consumer derives a set of subject keywords from the work in scope — the concrete paths, filenames, tools, commands, and area/acceptance-criteria terms it touches — and `grep`s the file for the lines matching them (case-insensitive; a single alternation or one pass per keyword), so only the matching atomic entries enter context. Because each entry names a single subject (a concrete path, tool, command, or setting), a subject keyword matches the lines whose gotcha is about that subject. No file, or no matching entries → the consumer carries no lessons block. Consumer-specific selection policies on top of this (e.g. plan's cap of 5 ticket-relevant entries) live in the consumer's own skill.
