...
2. Open the integration PR. Never paste ticket text into a quoted command literal (see `skills/build/references/pr-creation.md` for the same hardening): load the title via command substitution — `EPIC_TITLE=$(sed -n 's/^title: *//p' <epic-folder>/prd.md | head -1)` — write the summary (the N tickets shipped, each per-ticket PR #, the per-ticket review outcomes) to a temp file, then `gh pr create --base <base> --head integration/<epic-id> --title "<EPIC-ID>: $EPIC_TITLE" --body-file <summary-file>`.
