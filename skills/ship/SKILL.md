...
allowed-tools:
  ...
  - TodoWrite
  - Task
argument-hint: "[ticket-id ...] [--base branch] [--merge]"
---
...
orchestrator (main)
  ...
  └─ per ticket, in blocked_by order (epic) or list order (multi-solo):
       ...
       3. implementer subagent → read posted review → validate → fix → push → epic run: gh pr merge --squash into the integration branch; solo/multi-solo: leave the PR open
