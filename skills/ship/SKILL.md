...
**Environment requirement (applies to all of `ship`).** The orchestrator delegates building to an **implementer subagent**, and that implementer runs `feature:flow → build`, which itself spawns build's reviewer subagents from within — a nested subagent tree. The harness must support nesting to depth 2, observation of async agents, and at least four total agent threads. Build schedules its four reviewers in two waves of two, so the peak is root orchestrator + implementer + two reviewers; ship can then verify every hop as workers return.
...
2. Pre-flight: `git checkout <base> && git pull --ff-only` (default `main`), confirm a clean tree and `gh auth status` is logged in. Read the repo's CLAUDE.md for the project's load-bearing constraints. For carry-forward lessons, consume `claudedocs/tickets/_lessons.md` per the shared contract in [`../flow/references/lessons-log.md`](../flow/references/lessons-log.md) §8 — grep it by the subject keywords the shipped tickets touch, never full-load it — and pull only the matching atomic entries into the per-ticket brief; no file, or no match → carry no lessons block.
