...
- **Format overflow.** A lesson that can't fit one sentence on one topic is not a lesson — it's a durable rule. Propose it as a project-`CLAUDE.md` edit instead (same content-surfacing confirmation — show the exact text and target, never a bare yes/no); on decline — or in an unattended run, where the proposal is skipped — capture a one-sentence compression in `_lessons.md` instead — never silently lose the fact.
