...
- Preserve the leading header line (`# Lessons learned across tickets`) and any non-`^## ` lines **byte-for-byte** — the sweep only ever touches `^## ` entry lines. This mirrors build's "lines not matching `^## ` are preserved" rule.
